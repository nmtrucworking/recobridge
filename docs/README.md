...
1. **Kiến trúc tích hợp:** context, container, data flow và sequence diagram; giải thích lựa chọn REST so với gRPC và message broker.
2. **Tính toàn vẹn dữ liệu:** timeout, retry có điều kiện, circuit breaker, idempotency, transaction và cơ chế chống dual-write.
3. **Hiện thực hóa:** hợp đồng API, Docker Compose tham chiếu, kịch bản demo không dùng dữ liệu hard-code, test cases và bằng chứng cần thu thập.
...
- Sơ đồ container và deployment khớp Docker Compose thực tế.
